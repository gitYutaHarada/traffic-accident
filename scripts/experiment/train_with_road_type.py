...
def preprocess_with_road_type(df_original, n_clusters=50):
    df = df_original.copy()
    
    # すでに road_type に変換済みのデータセットを使用するため、変換処理は不要
    print("前処理: データセットは既にroad_type変換済みです")
    
    # road_typeをカテゴリとして確実に扱う
...
def save_results(metrics_df, feature_importances, output_dir='results/experiment_road_type'):
    ...
    plt.title('Feature Importance (Road Type Dataset)')
    plt.tight_layout()
    plt.savefig(f'{output_dir}/feature_importance.png')
    print(f"\nSaved results to {output_dir}")

def main():
    # 修正: 新しく作成した road_type データセットを読み込む
    input_file = r"c:\Users\socce\software-lab\traffic-accident\data\processed\honhyo_clean_road_type.csv"
    print(f"Loading data: {input_file}")
    df_raw = pd.read_csv(input_file)
    
    df_processed = preprocess_with_road_type(df_raw, n_clusters=50) 
