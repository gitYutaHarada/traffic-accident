...
def preprocess_with_interactions(df_original, n_clusters=50):
    df = df_original.copy()
    ...
    # クロス特徴量の作成
    print("前処理: クロス特徴量 (Interactions) を生成中...")
    
    # 1. 年齢(A) x 時間帯
    # 欠損値は 'nan' という文字列になるが、カテゴリカルとしては問題ない
    df['interaction_age_hour'] = (df['年齢（当事者A）'].astype(str) + '_' + df['hour'].astype(str)).astype('category')
    
    # 2. 天候 x 路面状態
    df['interaction_weather_road'] = (df['天候'].astype(str) + '_' + df['路面状態'].astype(str)).astype('category')
    
    # 3. 地形 x 道路形状
    df['interaction_terrain_shape'] = (df['地形'].astype(str) + '_' + df['道路形状'].astype(str)).astype('category')
    
    # 4. 車種(A) x 速度規制(A)
    df['interaction_type_speed'] = (df['当事者種別（当事者A）'].astype(str) + '_' + df['速度規制（指定のみ）（当事者A）'].astype(str)).astype('category')
    
    # 5. エリアID x 昼夜
    df['interaction_area_daynight'] = (df['area_id'].astype(str) + '_' + df['昼夜'].astype(str)).astype('category')
...
def train_and_evaluate(df):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X, y)):
        ...
        metrics.append([fold+1, accuracy, prec, rec, f1, auc])
...
def save_results(metrics_df, feature_importances, output_dir='results/experiments/interactions'):
...
def main():
    ...
    df_processed = preprocess_with_interactions(df_raw, n_clusters=50) # n=50使用
